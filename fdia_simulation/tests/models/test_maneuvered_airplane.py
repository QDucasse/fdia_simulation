...
import unittest
from fdia_simulation.models import Command, ManeuveredAirplane
...
class AirplaneTestCase(unittest.TestCase):
    ...
    def test_initial_x(self):
        self.assertEqual(self.airplane_test.x,0)

    def test_initial_y(self):
        self.assertEqual(self.airplane_test.y,0)

    def test_initial_z(self):
        self.assertEqual(self.airplane_test.z,0)

    def test_initial_vel(self):
        self.assertEqual(self.airplane_test.vel,0.3)

    def test_initial_headx(self):
        self.assertEqual(self.airplane_test.headx,0)

    def test_initial_headz(self):
        self.assertEqual(self.airplane_test.headz,0)
    ...
    def test_initial_headingx_command_in_commands(self):
        self.assertEqual(self.airplane_test.commands['headx'],Command('headx',0,0,0))
    ...
    def test_change_command_with_headingz(self):
        self.airplane_test.change_command('headz',15,20)
        cmd_headz = self.airplane_test.commands['headz']
    ...
    def test_change_command_with_velocity(self):
        self.airplane_test.change_command('vel',5,20)
        cmd_vel  = self.airplane_test.commands['vel']
        self.assertEqual(cmd_vel.value,5)
        self.assertEqual(cmd_vel.steps,20)

    def test_change_headx_command_effect(self):
        self.airplane_test.change_command('headx',5.,20)
        cmd_headx = self.airplane_test.commands['headx']
        for _ in range(20):
            self.airplane_test.update()
        self.assertEqual(self.airplane_test.headx,5.)
        self.assertEqual(cmd_headx.value,5.)
        self.assertEqual(cmd_headx.steps,0.)

    def test_change_headz_command_effect(self):
        self.airplane_test.change_command('headz',5.,20)
        cmd_headz = self.airplane_test.commands['headz']
        for _ in range(20):
            self.airplane_test.update()
        self.assertEqual(self.airplane_test.headz,5.)
